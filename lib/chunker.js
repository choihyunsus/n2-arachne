// chunker.js — 소스 코드를 함수/클래스 단위 청크로 분할
// Phase 1: 정규식 기반 (파서 불필요), Phase 2: AST로 교체 가능

/**
 * 토큰 수 추정 (간이 — 정확한 토크나이저 없이)
 * 코드 기준: ~3.5글자 = 1토큰
 */
 ...
 * @param {string} language - 언어 (js, ts, py, rs, go)
...
function chunkCode(content, language) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (const pattern of patterns) {
            ...
            break; // 이 라인은 매칭됨, 다음 라인으로
        }
    }

    // 청크에 포함되지 않은 코드가 있으면 "나머지" 청크로 추가
    const uncovered = [];
    let rangeStart = null;
    for (let i = 0; i < lines.length; i++) {
        if (!usedLines.has(i)) {
...
/**
 * 파일 전체를 하나의 청크로 만들기 (지원 안 되는 언어 / 매칭 실패 시)
 */
...
function buildSearchText(name, content, type) {
    ...
    // 코드에서 식별자 추출 (알파벳+숫자+_)
    const identifiers = content.match(/[a-zA-Z_]\w{2,}/g) || [];
    const unique = [...new Set(identifiers)];
    parts.push(...unique.slice(0, 50)); // 상위 50개만
